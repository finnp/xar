...
function create (cwd, opts) {
  opts = opts || {}
  var compressionStream = function () { return new PassThrough() }
  if (opts.compression === 'gzip') compressionStream = zlib.createGzip
  var tocNode = [
  ...
  function collectFiles (file, parent, cb) {
    fs.lstat(file, function (err, stat) {
      ...
      if (stat.isDirectory()) {
        ...
        fs.readdir(file, function (err, files) {
          ...
          files
            ...
            .forEach(function (file) {
              collectFiles(file, node, function (err) {
                if (err) return cb(err)
                todo--
                if (todo === 0) cb()
              })
        ...
        start += stat.size // assuming the compression will actually result in a smaller size
      }
    })
  }
  collectFiles(cwd, tocNode, heapFinished)
